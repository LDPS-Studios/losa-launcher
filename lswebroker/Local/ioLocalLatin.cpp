...
// 	}
	...
	{ 
		MAX_CMD_LOOP = 2,
		MAX_CHANGE_CMD_LOOP = 2,

		TYPE_CHANGE_CMD_ID  = 0,
		TYPE_CHANGE_CMD_KEY = 1,
	};
...
bool ioLocalLatin::IsRightGameServerID( const CString &rszGameServerID )
{
...
DWORD ioLocalLatin::GetResourceIDBack()
{
	return IDB_BACK_US;
}

DWORD ioLocalLatin::GetResourceIDChange()
{
	return IDB_CHANGEBTN_TH;
}

DWORD ioLocalLatin::GetResourceIDSetup()
{
...
DWORD ioLocalLatin::GetResourceIDErrorExit()
{
	return IDB_EXITBTN_TH;
}

DWORD ioLocalLatin::GetResourceIDErrorSolution()
{
	return -1;
}
...
void ioLocalLatin::SetRegKey( const char *szReg )
{
	StringCbCopy( m_szRegKey, sizeof( m_szRegKey ), szReg );
}

void ioLocalLatin::SetStartURL( const char *szURL )
{
	StringCbCopy( m_szStartURL, sizeof( m_szStartURL ), szURL );
}

bool ioLocalLatin::IsLoginDlg()
{
	return false;
}
