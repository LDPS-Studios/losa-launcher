...
void ioLoginDlg::OnBnClickedLogin()
{
	...
	if( m_bIDEditBlank || m_bPWEditBlank ) //아이디 또는 비밀번호가 비었을 경우
	{
		pMainFrame->ShowErrorDlg( "아이디 또는 패스워드를 입력해 주세요1", 108 );
		return;
	}
	...
	{
		pMainFrame->ShowErrorDlg( "아이디 또는 패스워드를 입력해 주세요2", 108 );
		return;
	}
	CString sPW;
	...
	{
		pMainFrame->ShowErrorDlg( "아이디 또는 패스워드를 입력해 주세요3", 108 );
		return;
	}
	...
	std::string mainkey = "losaapi";
	std::string apikey = "yourkey"; // put your key here 
	std::string auth_header = "Authorization: Bearer " + mainkey + apikey;
	...
	// Set the values for the username and password
	std::string username = sID;
	std::string password = sPW;

	// Set the JSON payload
	std::string payload = "{\"username\":\"" + username + "\", \"password\":\"" + password + "\"}";// you can customize your input key here
	...
	// Parse a JSON object from response
	...
	if (httpCode == 200) {
		...
		string losakey = "autoupgrade.exe EDEW3940FVDP4950,10,20,30,1,autoupgrade_info.ini,-1,0,1,119483910,?" + encodefix + encidfix + "?0?" + gameserverid + "?2020,09,10,0";
		
		StartProcess("autoupgrade.exe", losakey.c_str());
		...
		MessageBoxA(message->valuestring, "Error", MB_ICONERROR);
		return;
	}
...
{
	//system("start http://localhost/join/register.aspx");
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
